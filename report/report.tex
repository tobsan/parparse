...
% We want subsubsections to be numbered
...
\chapter{Background}
%TODO: Write something here. How to make this nice with introduction stuff?
...
% TODO: Caching, doing as little as possible when changes occur.
% TODO: Interactive system bla bla. Like makefiles work!
...
\]
% TODO: Describe the double arrow better (see parparse)
...
boolean matrices. 
% TODO: Describe, or at least mention the V function.
...
\end{figure}
% TODO: Read up on functional dependencies, paper by Jones (2000)
...
\end{figure}
% TODO: Look up the intToToken stuff. Is it even neccessary?
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \node[vertex] (st1) at (0,4.5) {\tiny Some-\\[-3mm]\tiny Tri};
        \node[vertex] (st2) at (2,4.5) {\tiny Some-\\[-3mm]\tiny Tri};
        \node[vertex] (st3) at (4,4.5) {\tiny Some-\\[-3mm]\tiny Tri};
        \node[vertex] (st4) at (6,4.5) {\tiny Some-\\[-3mm]\tiny Tri};
        ...
        % Mappend matrices
        \node[vertex] (st5) at (1,3) {\tiny Some-\\[-3mm]\tiny Tri};
        \node[vertex] (st6) at (5,3) {\tiny Some-\\[-3mm]\tiny Tri};
        \node[vertex] (st7) at (3,1.5) {\tiny Some-\\[-3mm]\tiny Tri};
        \draw[edge] (st1) -- (st5);
        \draw[edge] (st2) -- (st5);
        \draw[edge] (st3) -- (st6);
        \draw[edge] (st4) -- (st6);
        \draw[edge] (st5) -- (st7);
        \draw[edge] (st6) -- (st7);
        \node at (1,4) {\tiny $mappend$};
        \node at (5,4) {\tiny $mappend$};
        \node at (3,2.3) {\tiny $mappend$};
        % Output is an AST
        \node[rectangle, draw, text centered] (ast) at (3,-.5) {AST};
        \draw[open] (st7) -- (ast);
        ...
        \node[align=left] at (0.8,0.8) 
            {\tiny Result of parsing is a state \\[-3mm]
             \tiny of matrices (SomeTri) that \\[-3mm]
...
\end{figure}
% TODO: Clarify the table of lexing results for every possible input state.
% TODO: Use tikz to create this figure instead.
% TODO: Re-iterate the importance of caching here.
...
and recompute all but the leftmost elements when applying V. 
...
% TODO: Describe the _exact_ relation between the chopped matrix and the larger
% one (difference is always 1)
% TODO: Show implementation of chopShape
...
% TODO: Absolute performance and buckets/bucketing

\section{Testing}
% TODO: Grammar of the first sentence. Rewrite.
...
% TODO: Describe the measurements in more detail, both how they were run and
...
% TODO: Rewrite. V function has to be applied each time - so size matters. 
...
\begin{figure}[H]
% TODO: Redo this graphic with error bars etc
...
% TODO: Define and describe the branching
