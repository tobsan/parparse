...
% We want subsubsections to be numbered, and included in the ToC
...
file to get such information, but any real world use cases are beyond the scope
of this thesis.
...
\path{source/src/BNFC/Backend/Haskell/} directory in the same repository, where
...
\begin{subfigure}[H]{.45\textwidth}
    ...
    \begin{tikzpicture}
        ...
        \draw (0,2) -- (4,2);
        \draw (2,0) -- (2,4);
        \node at (3,3) {\Large $\{Tok\}$};
        \node at (1,3) {\Huge $Z$};
        \node at (1,1) {\Huge $Z$};
        \node at (3,1) {\Huge $Z$};
        \node at (2,4) [above = 1mm] {\textsc{Bin Leaf Leaf}};
        \node at (4,2) [rotate = -90, above = 1mm] {\textsc{Bin Leaf Leaf}};
    \end{tikzpicture}
...
\begin{subfigure}[H]{.45\textwidth}
    ...
    \begin{tikzpicture}
        ...
        \draw (0,4) -- (4,4);
        \draw (0,4) -- (4,0);
        \node at (2,2) {\fontsize{2cm}{1em}\selectfont $Z$};
        \node at (2,4) [above = 1mm] {\textsc{Bin Leaf Leaf}};
        \node at (4,2) [rotate = -90, above = 1mm] {\textsc{Bin Leaf Leaf}};
    \end{tikzpicture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \tikzset{
          vertex/.style = {shape=circle,draw,minimum size=1.5em, text centered, align=center},
          edge/.style = {->,> = latex'},
          open/.style = {->,> = open triangle 60}
        }
        % Input data
        \node[rectangle, draw, text centered] (input) at (3,16) {$x = 5;$};
        % Fingertree of chars
        \node[vertex] (a) at (3,15) {};
        \node[vertex] (b) at (1,14.5) {};
        \node[vertex] (c) at (5,14.5) {};
        \node[vertex] (x) at (0,14) {x};
        \node[vertex] (eq) at (2,14) {=};
        \node[vertex] (fi) at (4,14) {5};
        \node[vertex] (se) at (6,14) {$;$};
        \draw[open] (input) -- (a);
        \draw (a) -- (b);
        \draw (a) -- (c);
        \draw (b) -- (x);
        ...
        \draw (c) -- (fi);
        \draw (c) -- (se);
        % Measuring chars to token
        \node[vertex] (ls1) at (0,12.5) {\tiny Lex-\\[-3mm]\tiny state};
        \node[vertex] (ls2) at (2,12.5) {\tiny Lex-\\[-3mm]\tiny state};
        \node[vertex] (ls3) at (4,12.5) {\tiny Lex-\\[-3mm]\tiny state};
        \node[vertex] (ls4) at (6,12.5) {\tiny Lex-\\[-3mm]\tiny state};
        \draw[edge] (x) -- (ls1) node[midway, left] {\tiny $measure$};
        \draw[edge] (eq) -- (ls2) node[midway, left] {\tiny $measure$};
        \draw[edge] (fi) -- (ls3) node[midway, left] {\tiny $measure$};
        \draw[edge] (se) -- (ls4) node[midway, left] {\tiny $measure$};
        % Mappend lexstates
        \node[vertex] (ls5) at (1,11) {\tiny Lex-\\[-3mm]\tiny state};
        \node[vertex] (ls6) at (5,11) {\tiny Lex-\\[-3mm]\tiny state};
        \node[vertex] (ls7) at (3,9.5) {\tiny Lex-\\[-3mm]\tiny state};
        \node at (1,12) {\tiny $mappend$};
        \node at (5,12) {\tiny $mappend$};
        \node at (3,10.2) {\tiny $mappend$};
        \draw[edge] (ls1) -- (ls5);
        \draw[edge] (ls2) -- (ls5);
        \draw[edge] (ls3) -- (ls6);
        \draw[edge] (ls4) -- (ls6);
        \draw[edge] (ls5) -- (ls7);
        \draw[edge] (ls6) -- (ls7);
        % Fingertree of tokens
        \node[vertex] (d) at (3,8) {};
        \node[vertex] (e) at (1,7.5) {};
        \node[vertex] (f) at (5,7.5) {};
        \node[vertex] (t1) at (0,6.5) {\tiny Tok\\[-3mm]\tiny TV x};
        \node[vertex] (t2) at (2,6.5) {\tiny Tok\\[-3mm]\tiny TS =};
        \node[vertex] (t3) at (4,6.5) {\tiny Tok\\[-3mm]\tiny TI 5};
        \node[vertex] (t4) at (6,6.5) {\tiny Tok\\[-3mm]\tiny TS ;};
        \draw[open] (ls7) -- (d);
        \draw (d) -- (e);
        \draw (d) -- (f);
        \draw (e) -- (t1);
        ...
        \draw (f) -- (t3);
        \draw (f) -- (t4);
        % Measuring tokens to matrices
        \node[vertex] (st1) at (0,4.8) {\tiny Some-\\[-3mm]\tiny Tri};
        \node[vertex] (st2) at (2,4.8) {\tiny Some-\\[-3mm]\tiny Tri};
        \node[vertex] (st3) at (4,4.8) {\tiny Some-\\[-3mm]\tiny Tri};
        \node[vertex] (st4) at (6,4.8) {\tiny Some-\\[-3mm]\tiny Tri};
        \draw[edge] (t1) -- (st1) node[midway, left] {\tiny $measure$};
        \draw[edge] (t2) -- (st2) node[midway, left] {\tiny $measure$};
        \draw[edge] (t3) -- (st3) node[midway, left] {\tiny $measure$};
        \draw[edge] (t4) -- (st4) node[midway, left] {\tiny $measure$};
        % Mappend matrices
        \node[vertex] (st5) at (1,3.2) {\tiny Some-\\[-3mm]\tiny Tri};
        \node[vertex] (st6) at (5,3.2) {\tiny Some-\\[-3mm]\tiny Tri};
        \node[vertex] (st7) at (3,1.8) {\tiny Some-\\[-3mm]\tiny Tri};
        \draw[edge] (st1) -- (st5);
        \draw[edge] (st2) -- (st5);
        \draw[edge] (st3) -- (st6);
        \draw[edge] (st4) -- (st6);
        \draw[edge] (st5) -- (st7);
        \draw[edge] (st6) -- (st7);
        \node at (1,4.2) {\tiny $mappend$};
        \node at (5,4.2) {\tiny $mappend$};
        \node at (3,2.5) {\tiny $mappend$};
        % Output is an AST
        \node[rectangle, draw, text centered] (ast) at (3,0) {AST};
        \draw[open] (st7) -- (ast);
        % Draw box around measuring parts
        \draw (-1,0.7) -- (7,0.7);
        \draw (-1,0.7) -- (-1,5.55);
        \draw (7,0.7) -- (7,5.55);
        \draw (-1,5.55) -- (7,5.55);
        ...
        \draw (-1,13.2) -- (7,13.2);
        \draw (7,13.2) -- (7,8.7);
        % Write text in the boxes
        \node[align=left] at (0.8,1.1) 
            {\tiny Result of parsing is a state \\[-3mm]
             \tiny of matrices (\texttt{SomeTri}) that \\[-3mm]
             \tiny contain what will be the AST};
        \node[align=left] at (0.6,9.1)
            {\tiny Result of lexing is a state \\[-3mm]
             \tiny contining a \texttt{FingerTree} \\[-3mm]
             \tiny of lexed tokens};
...
Pay attention to the width of the lines, used to show how constructors are
grouped inside the matrices. 
\begin{figure}[H]
\begin{subfigure}[t]{.3\textwidth}
  ...
  \begin{tikzpicture}
    ...
    \draw[very thin] (1,2) -- (1,4); 
    \draw[very thin] (0,3) -- (2,3);
    \node[inChart] at (1,4) {$q1$};

    % Upper right
    \draw[very thick] (2,4) -- (4,4);
    \draw[very thick] (4,2) -- (4,4);
    \draw[very thin] (2,3) -- (4,3);
    \draw[very thin] (3,2) -- (3,4);
    \node[inChart] at (3,4) {$q2$};

    % x-axis shape
    \node at (0,4) [above = 5mm, right = 0mm] {\tiny \textsc{Bin (Bin Leaf Leaf) (Bin Leaf Leaf)}};
    % y-axis shape
    \node at (0,0) [rotate = 90, above = 3mm, right = 0mm] 
        {\tiny \textsc{Bin (Bin Leaf Leaf) (Bin Leaf Leaf)}};
  \end{tikzpicture}
\caption{\small \texttt{Quad q1 q2 q3 q4}, q1-q4 are \texttt{Quad a b c d}} 
\end{subfigure}
\hfill
\begin{subfigure}[t]{.3\textwidth}
    \centering
    ...
    \end{tikzpicture}
\caption{\small \texttt{Quad r1 r2 r3 r4}, r1-r4 are \texttt{Row a b}}
\end{subfigure}
\hfill
\begin{subfigure}[t]{.2\textwidth}
    ...
    \end{tikzpicture}
\caption{\small \texttt{Col (Col a b) c)}}
\end{subfigure}
\caption{\small Example matrices with their \texttt{Shape}s written out, and the
\texttt{Mat} constructor used to create them}
\end{figure}

\subsubsection{Merging charts} \label{chopsection}
...
the \texttt{mergein} function, it is possible to merge matrices using an
additional single element as glue. Such an approach simplifies the merging a
lot, because elements are placed just above the diagonal and that means a single
element can fill the small void in the merged matrix, as illustrated in figure
\ref{mergein}.
...
\caption{\small Merging with middle element, as done by \citet{parparsepaper}}
\end{subfigure}
\hfill
\begin{subfigure}[t]{.45\textwidth}
    \begin{tikzpicture}
        ...
        \draw[dotted] (4,6) -- (8,6);
        \draw[dotted] (4,7) -- (8,7);
        \node[circle, draw, align=center, inChart] at (5,5) {\phantom{P}};
        ...
        \node at (8,2) [rotate=-90, above=0mm] {Right};
    \end{tikzpicture}
\caption{\small Merge without middle element -- the diagonal is broken!}
\end{subfigure}
\caption{\label{mergein} \small Merging with and without a single element as glue.}
\end{figure}
...
and recompute all but the leftmost elements when applying the V function from
Valiant's algorithm. 
...
\centering
    \begin{tikzpicture}
        ...
        \draw (2,3) -- (2,7);
        \draw (3,3) -- (3,7);
        \node[inChart] at (2,7) {$X$};
        \node[inChart] at (3,6) {$Y$};
        \node[inChart] at (4,5) {$Z$};
        % Chopped right input matrix
        \draw (4,1) -- (7,1);
        \draw (4,2) -- (7,2);
        \draw (5,0) -- (5,3);
        \draw (6,0) -- (6,3);
        \node[inChart] at (6,3) {$B$};
        \node[inChart] at (7,2) {$C$};
        % Bottom left empty matrix
        \draw[dotted] (0,1) -- (4,1);
        \draw[dotted] (0,2) -- (4,2);
        \draw[dotted] (0,3) -- (4,3);
        ...
        \draw[dotted] (5,3) -- (5,7);
        \draw[dotted] (6,3) -- (6,7);
        \draw[dotted, very thick] (4,4) -- (7,4); % Chopped row
        \draw[dotted] (4,5) -- (7,5);
        \draw[dotted] (4,6) -- (7,6);
        \node[circle, draw, align=center, inChart] at (5,4) {$A$};
        \node[rectangle, draw, fill=white] at (4,5) [right=1mm] {\small $V(Left, A, Right')$};
        \draw [thick, ->] (5,4) to (6.9,4.7);
        \node at (0,5) [rotate=90, above=0mm] {Left};
        \node at (7,1.5) [rotate=-90, above=0mm] {Right'};

    \end{tikzpicture}
\caption{\label{chopmerge} \small Successful merge without a middle element. The first
...
% TODO: Clarify that the bool corresponds to leftOf and rightOf of SomeTri
% matrix
\label{oraclesection}
...
the project repository under \path{src/Test/Javalette/BenchParser.hs}
\cite{tobsan-repo}.
